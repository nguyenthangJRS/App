...
window.addEventListener("DOMContentLoaded", function() {
    ...
    LevelSetting(0); // level 1 setting 
}, false);
...
function LevelSetting(index) {
    ...
    // セッションストレージkey = "tic_tac_toe_access"がある場合は初回ではない
...
function bearTurn() {
    ...
    while (bearTurnEnd === "0") {
        ...
        if(level === "3") {
            let randomLine = Math.floor(Math.random()*(lineRandom.length));
            if(lineRandom[randomLine].classList.contains('js-clickable')){
                gameOverFlg = isSelect(lineRandom[randomLine]);
                    bearTurnEnd = "1";
                    break;
            }
